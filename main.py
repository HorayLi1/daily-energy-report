...
def fetch_news_from_coze():
    if not COZE_PAT or not COZE_BOT_ID:
        raise Exception("缺少环境变量：COZE_PAT 或 COZE_BOT_ID，请检查配置")
    ...
    while True:
        ...
        print(f"[DEBUG]轮询retrieve status={status} 已等待{elapsed:.1f}s")
    ...
    # 合并所有 assistant 消息的 reasoning_content + content
    raw_text = ""
    for msg in msg_data.get("data", []):
    ...
    if not raw_text:
        raise Exception("没有找到 assistant 的回复内容")

    print(f"[DEBUG] 原始assistant文本:\n{raw_text}")

    # 精确提取第一个 JSON 对象或数组（比贪婪正则更稳健）
        ...
        while idx < len(text):
            if text[idx] in '{[':
                try:
                    ...
                    pass
            idx += 1
        raise Exception("未找到合法的JSON内容")

    try:
        news_data = extract_json(raw_text)
    except Exception as e:
        raise Exception(f"提取 JSON 失败: {e}, 内容={raw_text[:500]}")
